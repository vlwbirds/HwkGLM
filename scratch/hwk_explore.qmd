...
plots <- lapply(unique(combined_data$group), function(grp) {
  ...
  # Plot abundance vs. alarm percentage
  abundance_plot <- ggplot(group_abundance_data, aes(x = abundance, y = alarm_percentage)) +
    geom_point(size = 3) +
    geom_smooth(method = "lm", se = FALSE) +
  ...
  # Plot richness vs. alarm percentage
  richness_plot <- ggplot(group_richness_data, aes(x = richness, y = alarm_percentage)) +
    geom_point(size = 3) +
    geom_smooth(method = "lm", se = FALSE) +
    labs(title = paste("Richness vs. Alarm Percentage for", grp),
         x = "Richness",
         y = "Alarm Percentage") +
    theme_minimal()
  
  return(list(abundance_plot = abundance_plot, richness_plot = richness_plot))
})

# Print the plots
for (i in seq_along(plots)) {
  print(plots[[i]]$abundance_plot)
  print(plots[[i]]$richness_plot)
}
