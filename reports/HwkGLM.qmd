...
percent_competitive <- mean(hwk_subset$alarm[hwk_subset$Social_Index == 2], na.rm = TRUE) * 100
...
sample_competitve <- sum(hwk_subset$Social_Index == 2, na.rm = TRUE)
...
```{r}
# Assuming hwk_data is your data frame
library(ggplot2)
library(dplyr)

# Convert "alarm" column to numeric, replacing non-numeric values with NA
hwk_data$alarm <- as.numeric(as.character(hwk_data$alarm))
